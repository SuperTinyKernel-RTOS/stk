...
This example demonstrates STK-driven Wi-Fi networking on the **Raspberry Pi Pico 2 W** (RP2350, Arm Cortex-M33), serving a small web UI over lwIP's `httpd`/SSI/CGI application — both plain **HTTP and HTTPS** — and advertising itself over mDNS — all wired into STK's threading and event model instead of the original SDK example's single superloop.

It is a direct STK port of the Pico SDK's stock `pico_w` Wi-Fi httpd example: the same page content and CGI/SSI/POST handlers, but restructured so that Wi-Fi connect, server setup, and request handling run inside a proper STK task, driven by events rather than polled in a `while (true)`. On top of that, it adds TLS termination for the served pages via lwIP's `altcp_tls`, mbedTLS 4.x, and PSA Crypto, so the device can be reached at `https://<hostname>.local` (or its IP) in addition to plain `http://`.
...
- **HTTPS via `altcp_tls` + mbedTLS 4.x**: `LWIP_ALTCP` / `LWIP_ALTCP_TLS` / `LWIP_ALTCP_TLS_MBEDTLS` and `HTTPD_ENABLE_HTTPS` layer a second `httpd_inits()` listener on `HTTPD_SERVER_PORT_HTTPS` (443) on top of the same CGI/SSI handlers already registered for plain HTTP on port 80. `InitNetwork()` calls `psa_crypto_init()` right after `cyw43_arch_init()`, as required by mbedTLS 4's PSA-based crypto core, before the TLS config is built from a self-signed key/cert pair via `altcp_tls_create_config_server_privkey_cert()`. This requires the **STK fork of lwIP** (see prerequisites below), since upstream lwIP's `sys_arch`/threading layer doesn't target STK.
...
2. Clone the **STK fork of lwIP**, `stk` branch, into the `stk` repository's `deps` folder (e.g. `deps/lwip`) — it isn't bundled with `stk` and the project expects to find it there:
   ```
   git clone --branch stk https://github.com/SuperTinyKernel-RTOS/lwip deps/lwip
   ```
   This example needs the STK fork rather than upstream lwIP: it carries `sys_arch`'s STK-targeted threading/locking layer (`LWIP_STK_*` options in `lwipopts.h`) that upstream lwIP doesn't have. See the fork at `SuperTinyKernel-RTOS/lwip` — [`stk` branch](https://github.com/SuperTinyKernel-RTOS/lwip/tree/stk).
...
5. Generate the self-signed TLS certificate/key used by the HTTPS listener. An EC (`prime256v1`) key is recommended over RSA here — it's considerably smaller, which matters on a flash-constrained target like this one, and `mbedtls_config.h` already enables `MBEDTLS_ECP_DP_SECP256R1_ENABLED`/`MBEDTLS_ECDSA_C` for it:
   ```
   openssl req -x509 -newkey ec -pkeyopt ec_paramgen_curve:prime256v1 -keyout server_key.pem -out server_cert.pem -days 365 -nodes
   python generate_example_certs_h.py
   ``` 
   or RSA, if your project requires it:
   ```
   openssl req -x509 -newkey rsa:2048 -keyout server.key -out server.crt -days 365 -nodes -subj "/CN=localhost"
   python generate_example_certs_h.py
   ```

   `generate_example_certs_h.py` defaults to reading `server.key`/`server.crt` (its `KEY_FILE`/`CERT_FILE` constants) — update those to `server_key.pem`/`server_cert.pem` (or rename the generated files) to match the command above. The script converts either an RSA or EC PEM key to DER and emits `example_certs.h` (the `s_TlsKey`/`s_TlsCert` arrays), which the example includes directly and feeds to `altcp_tls_create_config_server_privkey_cert()`. Regenerate it if you want your own cert rather than the example's.
6. In Eclipse CDT (with the [Eclipse Embedded CDT](https://eclipse-embed-cdt.github.io) plugins), go to **File → Import → General → Existing Projects into Workspace** and select `build/example/project/eclipse/rpi/httpd_tls-rp2350w`.
7. Connect your Pico 2 W board via its SWD debug probe (e.g. Picoprobe/CMSIS-DAP) or another supported OpenOCD-compatible probe.
8. Select the **Debug** build configuration.
9. Build and start the debug session. The pre-build step regenerates `fsdata.c` from `content/`, the project compiles and links against the core STK sources, and the board is flashed and launched.

Once running, the board connects to Wi-Fi, starts the httpd/mDNS server on port 80 and the HTTPS listener on port 443, and prints its IP address (and `<hostname>.local` mDNS name) over the USB serial console. Point a browser at `http://` or `https://` for either address to reach the served page — the browser will warn that the HTTPS cert isn't CA-signed, which is expected for a self-signed cert — toggle the onboard LED via the form on the page, and watch the `uptime`/`ledstate` SSI tags update.
...
- [`SuperTinyKernel-RTOS/lwip`, `stk` branch](https://github.com/SuperTinyKernel-RTOS/lwip/tree/stk) — the STK-targeted lwIP fork this example (and its HTTPS support) builds against; clone this into `deps/lwip` as described above.
...
- `build/example/httpd_tls` — this example's sources, `content/` web assets, `makefsdata.py`, and `generate_example_certs_h.py`.
