...
static void InitSecureData()
{
    extern char __secure_data_start[];
    extern char __secure_data_end[];
    extern char __secure_data_source__[];

    // copy initialized variables from FLASH to RAM
    const size_t data_size = __secure_data_end - __secure_data_start;
    if (data_size > 0)
    {
        STK_MEMCPY(__secure_data_start, __secure_data_source__, data_size);
    }
}
PICO_RUNTIME_INIT_FUNC(InitSecureData, "00100");
...
static void CreateKernel()
{
    ...
    // Add Secure tasks:
    static SecureHwCommandQueueTask<ACCESS_PRIVILEGED> hw_cmd_task __stk_aligned(32U);
