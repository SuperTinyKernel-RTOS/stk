...
| **Memory Protection Unit (MPU) support**             | Privileged `ACCESS_PRIVILEGED` and non-privileged tasks `ACCESS_USER`, memory access partition - static and dynamic up to 3 per-task memory regions reprogrammed on every context switch                                     |
