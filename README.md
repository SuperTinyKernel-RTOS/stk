...
| **Memory Protection Unit (MPU) support**             | Privileged `ACCESS_PRIVILEGED` and non-privileged tasks `ACCESS_USER`, memory access partition - static and dynamic up to 6 per-task memory regions reprogrammed on every context switch                                     |
...
| **Per-task MPU stack guard (`STK_MPU_STACK_GUARD`)** | Hardware-enforced stack overflow protection, plus 2, 4 or 6 dynamic per-task regions (auto stack guard + task-instance/user-defined) reprogrammed on every context switch                                                    |
...
- **Regions 2-5 — free for application use, when `STK_MPU_TASK_REGIONS = 6`.** In 6-region mode, four additional slots are left entirely for the application to define via the same `GetMpuRegions()` list — e.g. a restricted peripheral window, a shared read-only code/data section, or a second private buffer. In 4-region mode, 2 regions are available. In 2-region mode, only region 1 is available.
- All region descriptors for the incoming task are computed once and written to the live MPU registers in a single burst load/store during the PendSV handler, so per-task region reprogramming stays a small, fixed-cost part of every context switch regardless of whether 2, 4 or 6 regions are configured.
...
| `sync::ConditionVariable`    | Monitor-pattern signaling used with an `IMutex`-compatible lock. Atomically releases the lock and blocks the caller; re-acquires it on wake. Supports `Wait()`, `NotifyOne()`, and `NotifyAll()`, plus critical-section variants `NotifyOne_CS()`/`NotifyAll_CS()`. `WaitEx()` reports a cancelled wait (via `IKernel::CancelTaskWait()`) distinctly from a timeout.                |
...
| `sync::Mutex`                | Recursive mutual exclusion primitive with priority inheritance (temporarily boosts the owner's priority to avoid inversion). Tracks ownership and recursion depth; the same task may lock multiple times. Ownership transfers directly to the first waiter (FIFO) on `Unlock()`.      |
| `sync::RWMutex`              | Reader-Writer Lock for shared (read) and exclusive (write) access. Implements a Writer Preference policy to prevent writer starvation. Non-recursive — a task must release before re-acquiring. Provides RAII guards `ScopedTimedLock` and `ScopedTimedReadMutex`.                    |
...
| `sync::Semaphore`            | Counting semaphore for resource throttling and signaling. Features a Direct Handover policy: `Signal()` passes the token directly to the first waiting task without touching the internal counter. `TrySignal()` posts without exceeding `max_count`, returning `false` instead of asserting. |
| `sync::Pipe`                 | Thread-safe, runtime-sized FIFO ring buffer over a caller-supplied external byte buffer. Parameterised on element size (bytes) rather than a C++ type; payloads transfer via `memcpy`, suitable for heterogeneous or C-ABI structs. Supports blocking/non-blocking single-element and bulk (`WriteBulk`/`ReadBulk`) I/O, plus `ReadBulkTriggered()` which blocks until a minimum element count is available before draining.                                    |
| `sync::PipeT<T, Capacity>`   | Thread-safe, compile-time-sized, type-safe FIFO pipe with internal storage. Parameterised on a concrete element type `T` and capacity; uses direct typed assignment for scalar types (and a per-element fallback otherwise). Zero dynamic memory allocation.                          |
| `sync::MessageQueue`         | Fixed-capacity, fixed-message-size FIFO queue backed by a caller-supplied byte buffer. Parameterised on a byte count rather than a type, making it suitable for C-ABI structs or heterogeneous payloads. Supports FIFO insertion/removal (`Put`/`Get`/`Peek`) and priority front-insertion (`PutFront`/`PeekFront`) for urgent messages.                                             |
| `sync::MessageQueueT<N, MSG>`| Compile-time-sized variant of `sync::MessageQueue` that owns its storage internally (capacity `N`, message size `MSG` bytes) — no external buffer required.                                                                                                                            |
| `sync::Barrier`              | Cyclic rendezvous point that blocks a fixed-size group of tasks until all of them have called `Wait()`, then releases the group and automatically resets for the next round. `WaitEx()` reports the full outcome (`BARRIER_LAST_ARRIVAL`, `BARRIER_RELEASED`, `BARRIER_CANCELED`). Built on `sync::Mutex` and `sync::ConditionVariable`.                                            |
